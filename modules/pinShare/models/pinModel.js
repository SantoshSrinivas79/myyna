...
var pinModel = {
    Pinlists: function(user_id, callback) {
        ...
        collection.find({'blocked':0}).limit(15).sort({
            time: -1
        }, function(err, res) {
    ...
    ////List pins
    //    Pinlists1: function(user_id, callback) {
    //        var collection = mongodb.collection('pins');
    ...
    //                        });
    //
    //                        mongodb.collection('pin_like').findOne({'user_id': mongo.ObjectID(user_id), 'pin_id': v._id}, function(err, res3) {
    //                            j++;
    //
    //                            if (res3) {
    //                                v.pinlike = 0;
    ...
    getAllpinsbyBoard: function(board_id, callback) {
        ...
        var collection = mongodb.collection('pins');
        collection.find({
            'board_id': mongo.ObjectID(board_id),'blocked':0
        }), function(err, res) {
            res.toArray(function(er, data) {
                console.log(data);
                callback(data);
        ...
        },
        {
            ...
            data.forEach(function(v) {
                ...
                mongodb.collection('pins').findOne({
                    _id: mongo.ObjectID(id),'blocked':0
                }, function(err, res2) {
    ...
    morePinlists: function(user_id, start, end, list,list_id, callback) {
        ...
        else if (list == "user")
        {
            ...
            var collection = mongodb.collection('pins');
            collection.find({
                user_id: mongo.ObjectID(creator_id),'blocked':0/*,repin:{$ne:1}*/
                    ...
                    res.toArray(function(er, data) {
                        if (data.length > 0)
                        {
                            ...
                            data.forEach(function(v) {
                                ...
                                v.loggeduser_id = user_id;
                                mongodb.collection('board').findOne({
                                    _id: mongo.ObjectID(id),locked:0
                                }, function(err, res2) {
        ...
        else if(list=='board')
        {
            ...
            var collection = mongodb.collection('pins');
            collection.find({
                board_id: mongo.ObjectID(board_id),'blocked':0/*,repin:{$ne:1}*/
            }).skip(start).limit(end).sort({
                time: -1
            }, function(err, res) {
                res.toArray(function(er, data) {
    ...
    getPinDetails: function(pin_id, callback) {
        ...
        var collection = mongodb.collection('pins');
        collection.find({
            '_id': mongo.ObjectID(pin_id),'blocked':0
        }, function(err, res) {
            res.toArray(function(er, data) {
                callback(data);
            });
    ...
    //get the details of pin creator
    ...
    getUserPincount:function(user_id,callback){
       ...
       var collection = mongodb.collection('pins');
        collection.find({
            'user_id': mongo.ObjectID(user_id),'blocked':0
        }, function(err, res) {
            res.toArray(function(er, data) {
    ...
    UserFollowerCount :function(id,callback){
       ...
       collection.find({ 'follower_id': mongo.ObjectID(id),'type':'user' },function (err, res) {
                 res.toArray(function(er, data){
                   console.log(data);
                    callback(data);
                });
